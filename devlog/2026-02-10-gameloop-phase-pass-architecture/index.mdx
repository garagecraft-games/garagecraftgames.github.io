...
This worked well in simple scenarios, but forced all events to wait until the next frame to be readable. That did not work, especially for systems that needed to communicate in the same logical phase. The new game loop architecture introduces a two-level hierarchy:
